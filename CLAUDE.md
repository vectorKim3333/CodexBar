...
- **외장 모니터 연결/해제 시 status bar overflow (1.5.9 사건).** 사용자가 외장 모니터 ↔ 맥북 내장 화면 사이를 전환하면 메뉴바 폭이 변함. macOS 가 폭 부족 시 overflow 항목을 hide 하는데 모니터 다시 연결해도 자동으로 안 돌려놓음. `NSApplication.didChangeScreenParametersNotification` 옵저버는 둘 다 컨트롤러에 있어야 하고 (`StatusItemController` + `CompanionStatusItemController`), screen change cascade (750ms + 3s + 10s) 로 단발 check 가 못 잡는 케이스 cover. `checkScreenChangeStatusItemVisibility` 가 screen count *증가* (외장 모니터 연결) 도 무조건 recovery 발화하도록 — 1.5.9 까진 감소 시만 적극 처리해서 외장 모니터 재연결 후 hide stuck 못 풀렸음. image cache (`lastAppliedProviderIconRenderSignatures`) 도 screen change 시 invalidate 해야 다음 updateIcons 가 새 image 강제 그림.
