...
- **최초 실행 시 캐릭터 안 보임 = 메뉴바 overflow/노치 클리핑 (1.8.7 사건, 미확정 — 로그로 최종 확인 권장).** 팀원이 처음 설치했는데 캐릭터(Companion)가 안 보이고 on/off·아이콘 복구(=재시작)·모니터 분리/재연결 어떤 방법으로도 안 나오는 보고. 코드 사실: status item 생성 순서가 `ensureStatusController()`(pill 2개) → `setupCompanionControllerIfPossible()`(캐릭터) 라 **캐릭터가 마지막 생성 = macOS 가 노치에 가장 가까운 최좌측에 배치 = 폭 부족 시 제일 먼저 노치 뒤로 잘림** (pill 은 오른쪽이라 살아남음 → "pill 은 보이는데 캐릭터만 안 보임"의 정체). `start()` 는 `variableLength` 만, 특정 NSScreen 지정 없음. 이건 1.8.2 가 적은 **mode-2 "hidden" (window/screen/width/image 전부 정상인데 안 보임)** 으로, `isStatusItemHealthy`/`isBlockedSnapshot` 어떤 속성 검사로도 감지 불가 → 자동/수동 복구·재시작 전부 무력 (재시작해도 같은 좁은 메뉴바 같은 위치로 재등록). **즉 1.8.2 의 "manual recover(=항상 재시작) 가 hidden 도 100% 복구" 주장은 genuine overflow 엔 거짓이다 — 재시작은 메뉴바 폭을 늘려주지 못함.** 코드로 강제 표시 불가 (macOS 한계). 1.8.7 완화 (no-regret): (1) 세 status item 모두 `autosaveName` 부여 (`StatusItemController.lazyStatusItem` 의 `"ClCoBar.pill.<provider>"`, `CompanionStatusItemController.start()` 의 `"ClCoBar.companion"`) → 사용자가 ⌘-드래그로 노치 밖으로 빼낸 위치가 잦은 재시작(디스플레이 변경/heartbeat) 후에도 유지 (이전엔 매 재시작마다 초기화돼 다시 잘림). (2) 설정 화면 캐릭터 섹션에 안내문 (`companion.notVisible.hint`). **확정에 필요한 결정적 증거: 팀원의 `~/Library/Logs/CodexBar/CodexBar.log` geometry 덤프** — `screen=nil` 이면 orphan(코드 re-home 가능), `screen` 정상+`width>0` 인데 안 보이면 overflow/노치 확정(macOS 한계, 우회만). 로그 없이 detection/recreate 류 "진짜 fix" 를 시도하지 말 것 — 1.5.5~1.8.0 의 11패치 헛발과 동일한 함정.
- **신규/미설정 사용자가 매 refresh·재시작마다 브라우저 쿠키 스캔 → TCC 프롬프트 반복 (1.8.7 사건, 1.8.4/1.8.5 의 잔여 누수).** 1.8.4 가 `needsWebProbe` 게이트로 OAuth 사용자의 스캔은 막았지만 `(!hasOAuth && !hasCLI)` 절은 그대로 두었다 — OAuth·CLI 자격증명이 둘 다 없는(=설치 직후 미설정) 사용자는 Web 이 유일한 fallback 이라 **매 refresh 마다 `ClaudeWebFetchStrategy.isAvailableForFallback` → `hasSessionKey(browserDetection:)` → `extractSessionKeyInfo` 로 브라우저 쿠키 DB 를 스캔**. `CookieHeaderCache` 는 **성공한 헤더만** 캐시하므로 세션이 *없는* 사용자는 영원히 캐시 미스 → 무한 재스캔. ad-hoc 서명이라 TCC grant 가 프로세스 간 비영구 → 모니터 변경 재시작(cold-start) 마다 다시 프롬프트. "여러 번"의 또 다른 축: Claude·Codex 가 각각 별도 스캔. 1.8.7 fix: `ClaudeWebProbeGate` (UserDefaults `claude.autoWebProbe.negativeUntil`, TTL 1h) — `makePlanningInput` 에서 **자동 fallback 경로만** negative 캐시로 게이팅 (세션 못 찾으면 1h 재스캔 차단, 찾으면 즉시 해제 self-heal). 명시적 opt-in (manual header / `.web` 소스 / webExtras) 은 항상 probe (`explicitWebNeed` 분기). `BrowserCookieAccessGate` 의 6h keychain 거부 cooldown 과 같은 발상이되 "세션 없음"을 기억. (Codex OpenAI web extras 는 별도 opt-in 경로 — 이번 fix 범위 밖, 기본 off 면 무관.)
...
- **신규 Claude 모델 출시 시 "주요 모델"/비용 $0 버그 (1.8.6 Opus 4.8 → 1.8.9 Fable 5 재발).** 가격표(`CostUsagePricing.swift` 의 `claude` dict)에 새 모델 단가가 없으면 비용이 $0 으로 계산돼 "주요 모델"(=최대 costUSD)에서 누락되고 일일 비용도 과소 계상된다. models.dev 캐시(`~/Library/Caches/CodexBar/model-pricing/models-dev-v1.json`)가 가격표보다 우선이지만 신모델 반영이 느려 못 믿는다. 1.8.9 부터 3중 방어: ① family fallback (opus/sonnet/haiku/fable 분기) ② **`claude` prefix 모델 전체에 flagship 단가 최후 fallback** — 어떤 새 family 가 나와도 $0 은 안 됨 (과대 추정 가능하나 $0 보다 낫고, 캐시는 `pricedSampleCount<sampleCount` 면 자동 재계산되므로 캐시 삭제 불필요). ③ 그래도 **새 모델이 나오면 할 일**: `claude` dict 에 정확한 단가 추가 (claude-api 스킬에서 단가 확인) + flagship 이 바뀌면 최후 fallback 대상 갱신 + `CompanionMenuBuilder.shortModelName` 의 families 에 새 family 표시명 추가. 검증은 `~/Library/Caches/CodexBar/cost-usage/claude-v2.json` 의 days 에서 해당 모델 packed 배열의 cost(5번째)·pricedSampleCount(7번째) 가 0 이 아닌지 확인.

- **환경설정 창 여는 셀렉터는 `showSettingsWindow:` (macOS 13+), `showPreferencesWindow:` 아님 (1.8.10 사건 — 실측 확정).** macOS 13(Ventura)부터 SwiftUI `Settings` scene 의 AppKit responder 가 `showPreferencesWindow:` → `showSettingsWindow:` 로 개명됐다. macOS 26 에서 `NSApp.sendAction(Selector(("showPreferencesWindow:")), to: nil, from: nil)` 는 **응답자가 없어 `false` 반환 = 창 안 뜸** (계측 빌드로 확인: `showPreferencesWindow:=false showSettingsWindow:=true`). **일상 경로는 안전** — pill/Companion 메뉴의 "설정" 은 `.codexbarOpenSettings` 노티 → `HiddenWindowView` 의 `@Environment(\.openSettings)` env action 을 쓰고, 이 keepalive `WindowGroup` 창은 메뉴바 status item 과 **독립적**이라 아이콘이 숨겨진 상태에서도 동작한다 (relaunch 후에도 재생성됨 — 실측). 깨졌던 건 `applicationShouldHandleReopen` (아이콘 다 사라졌을 때 Finder 재실행 catch-22 escape) 과 죽은 코드 `CodexBarApp.openSettings` 뿐. 게다가 예전 reopen 은 `forceRecoverAllMenuBarItems()` 로 500ms 뒤 process 를 relaunch 시켜, 설정 창이 떠도 프로세스가 죽으면서 사라뜨렸다 (+ 1.8.5 TCC 프롬프트 유발). **교훈: (1) 새 셀렉터 쓸 땐 `showSettingsWindow:` 우선, `showPreferencesWindow:` 는 <13 폴백. (2) reopen 에서 relaunch 하지 말 것 — 설정만 확실히 열고(노티 경로 + 셀렉터 폴백) 아이콘 복구는 설정 안의 '메뉴바 아이콘 복구' 버튼(명시적 relaunch)에 맡긴다. (3) 설정 창은 다른 앱 뒤/사라진 모니터의 저장 위치(화면 밖)에 열릴 수 있으니 열릴 때 `PreferencesView.presentSettingsWindowToFront()` 로 항상 앞으로 + 화면 안으로 당긴다.**
