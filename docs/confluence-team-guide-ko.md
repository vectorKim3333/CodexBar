...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| **1.8.10** | 2026-07-07 | **환경설정(설정) 창이 안 열리던 문제 근본 수정 (macOS 13+/Tahoe).** 메뉴바 아이콘이 다 사라졌을 때 Finder/Spotlight 에서 앱을 다시 열어 설정으로 탈출하는 경로(catch-22 escape)가 macOS 26 에서 완전히 깨져 있었음. 원인 2가지: (1) 설정 창을 여는 AppKit responder 셀렉터가 macOS 13(Ventura)부터 `showPreferencesWindow:` → **`showSettingsWindow:`** 로 개명됐는데 옛 이름을 써서 응답자가 없어(`sendAction`=false) 창이 안 떴음(실측 확인). (2) 그 경로가 **500ms 뒤 앱을 자동 재시작**(`forceRecoverAllMenuBarItems`)시켜, 설령 창이 떠도 프로세스가 죽으면서 사라졌음(1.8.5 의 TCC "다른 앱 데이터 접근" 프롬프트도 유발). 수정: 재시작 없이 **설정 창만 확실히 연다** — keepalive 창을 통한 SwiftUI 경로(메뉴바 아이콘과 독립적이라 아이콘이 숨겨진 상태에서도 동작) + macOS 버전에 맞는 셀렉터 폴백. 또한 설정 창이 **다른 앱 뒤/사라진 모니터의 저장 위치(화면 밖)에 열려 안 보이던** 케이스 대비로 열릴 때 항상 앞으로 + 화면 안으로 당겨온다(일상 메뉴 경로에도 적용). **동작 변화:** Finder 재실행이 더 이상 자동 재시작(아이콘 복구)을 하지 않는다 — 대신 설정이 확실히 열리므로 그 안의 "메뉴바 아이콘 복구" 버튼으로 복구(= 명시적 재시작, 설정 진입과 분리). 죽은 코드였던 `CodexBarApp.openSettings` 도 제거. |
| **1.8.9** | 2026-06-10 | "주요 모델"이 새 모델(Fable 5) 사용 중에도 옛 모델(Opus 4.8)로 표시되던 버그 수정 — 1.8.6 과 같은 패턴의 재발. 가격표에 `claude-fable-5` 단가($10/$50 per MTok) 누락으로 Fable 5 비용이 $0 으로 계산돼 "주요 모델"(=최대 비용 모델) 산정에서 빠지고 오늘 비용도 과소 계상됐음. 1.8.6 의 family fallback 이 opus/sonnet/haiku 만 커버해 새 family(fable)는 구멍이었음. 수정: `claude-fable-5` 단가 추가 + fallback 에 fable 분기 + **완전히 새로운 family 도 $0 이 되지 않게 flagship 단가 최후 fallback** 추가(앞으로 어떤 신규 모델이 나와도 $0 재발 없음). 메뉴 표시명 "Fable 5" 매핑도 추가. |
| **1.8.8** | 2026-06-09 | **최초 실행 시 기본 활성화 provider 변경.** 예전엔 Codex 만 켜진 채 시작했으나, 이제 **Claude·Codex 둘 다 기본 활성화**된다(`ClaudeProviderDescriptor.defaultEnabled` false→true). 앱 취지(Claude+Codex 사용량 추적)에 맞춰 첫 설치 시 두 pill 이 모두 보인다. 이미 설정 파일(`~/.codexbar/config.json`)이 있는 기존 사용자는 영향 없음(기존 on/off 유지). |
