...
> This is a Claude+Codex-only slim fork of CodexBar. CLI, Widget, Sparkle auto-update, and 40+ other AI providers have been removed.
...
- `Scripts`: build/package helpers (`package_app.sh`, `build_icon.sh`, `compile_and_run.sh`).
- `docs`: developer reference docs (architecture, configuration, provider details, troubleshooting).
...
- Package locally: `./Scripts/package_app.sh` to refresh `CodexBar.app`, then restart with `pkill -x CodexBar || pkill -f CodexBar.app || true; open -n <repo-root>/CodexBar.app`.
